...
class yolo:
    ...
    def _yolo_block(self, inputs, filters_num, out_filters, conv_index, training=True, norm_decay=0.99, norm_epsilon=1e-3):
        ...
        conv = self.batch_normalization_layer(conv, name='batch_normalization'+str(conv_index), training=training, norm_decay=norm_decay, norm_epsilon=norm_epsilon)
        conv_index += 1
        conv = self._conv2d_layer(inputs, filters_num=filters_num*2, kernel_size=3, strides=1, name='conv2d'+str(conv_index))
        conv = self.batch_normalization_layer(conv, name='batch_normalization'+str(conv_index), training=training, norm_decay=norm_decay, norm_epsilon=norm_epsilon)
        conv_index += 1
    ...
    def _yolo_inference(self, inputs, num_anchors, num_classes, training=True):
        ...
        with tf.variable_scope('yolo'):
            ...
            conv2d_60 = self.batch_normalization_layer(conv2d_60, name='batch_normalization'+str(conv_index), training=training, norm_decay=self.norm_decay, norm_epsilon=self.norm_epsilon)
            conv_index += 1
            # 特征图上采样
            ...
            conv2d_68 = self.batch_normalization_layer(conv2d_68, name='batch_normalization'+str(conv_index), training=training, norm_decay=self.norm_decay, norm_epsilon=self.norm_epsilon)
            conv_index += 1
            # 特征图上采样
