import numpy as np
import random
from random import shuffle
from PIL import Image
import tensorflow as tf
from matplotlib.colors import rgb_to_hsv, hsv_to_rgb
from utils.anchors import get_anchors
import keras
from keras.applications.imagenet_utils import preprocess_input
from keras import backend as K
from keras.objectives import categorical_crossentropy

# 1-随机生成函数在[a, b)区间的随机数
def rand(a=0, b=1):
    return np.random.rand()*(b-a)+a

# 2-分类损失函数
def cls_loss(ratio=3):

    # 01-定义计算分类损失函数
    def _cls_loss(y_ture, y_pred):
        labels = y_ture
        anchor_state = y_ture[:, :, -1]
        classifcation = y_pred

        # 计算目标先验框的分类损失
        indices_for_object = tf.where(keras.backend.equal(anchor_state, 1))  # 获取目标的索引
        labels_for_object = tf.gather_nd(labels, indices_for_object)  # 获取对应标签
        classifcation_for_object = tf.gather_nd(classifcation, indices_for_object)  # 获取分类结果
        cls_loss_for_object = keras.backend.binary_crossentropy(labels_for_object, classifcation_for_object)  # 计算分类损失

        # 计算背景先验框的分类损失
        indices_for_back = tf.where(keras.backend.equal(anchor_state, 0))
        labels_for_object = tf.gather_nd(labels, indices_for_back)
        classifcation_for_back = tf.gather_nd(classifcation, indices_for_back)
        cls_loss_for_back = keras.backend.binary_crossentropy(labels_for_object, classifcation_for_back)

        # 计算正样本和负样本的数量，确保最小为1，以避免除以0
        normalizer_pos = tf.where(keras.backend.equal(anchor_state, 1))
        normalizer_pos = keras.backend.cast(keras.backend.shape(normalizer_pos)[0], keras.backend.floatx())
        normalizer_pos = keras.backend.maximum(keras.backend.cast_to_floatx(1.0), normalizer_pos)
        normalizer_neg = tf.where(keras.backend.equal(anchor_state, 0))
        normalizer_neg = keras.backend.cast(keras.backend.shape(normalizer_neg)[0], keras.backend.floatx())
        normalizer_neg = keras.backend.maximum(keras.backend.cast_to_floatx(1.0, normalizer_neg))

        # 对正样本和负样本的损失分别归一化，然后加权求和
        cls_loss_for_object = keras.backend.sum(cls_loss_for_object)/normalizer_pos
        cls_loss_for_back = ratio * keras.backend.sum(cls_loss_for_back)/normalizer_neg
        loss = cls_loss_for_back + cls_loss_for_object

        return loss

    return _cls_loss

# 02-定义smooth_l1损失函数
def smooth_l1(sigma=1.0):
    sigma_square = sigma ** 2

    def _smooth_l1(y_true, y_pred):
        # 提取回归目标和状态信息
        regression = y_pred
        regression_target = y_true[:, :, -1]
        anchor_state = y_true[:, :, -1]

        # 找出正样本的索引
        indices = tf.where(keras.backend.equal(anchor_state, 1))
        regression = tf.gather_nd(regression, indices)
        regression_target = tf.gather_nd(regression_target, indices)

        # 计算smooth_l1损失
        regression_diff = regression - regression_target
        regression_diff = keras.backend.abs(regression_diff)
        # 计算预测值和真实值之间的差异，使用tf.where来选择平方损失或线性损失
        regression_loss = tf.where(
            keras.backend.less(regression_diff, 1.0 / sigma_square),
            0.5 * sigma_square * keras.backend.pow(regression_diff, 2),
            regression_diff - 0.5 / sigma_square)

        # 归一化损失
        normalizer = keras.backend.maximum(1, keras.backend.shape(indices)[0])
        normalizer = keras.backend.cast(normalizer, dtype=keras.backend.floatx())
        loss = keras.backend.sum(regression_loss) / normalizer

        return loss

    return _smooth_l1

# 03-定义分类损失函数，计算目标检测中的回归损失
def class_loss_regr(num_classes):
    epsilon = 1e-4
    # 定义内部损失函数，用于计算分类回归损失
    def class_loss_regr_fixed_num(y_true, y_pred):
        x = y_true[:, :, 4*num_classes:] - y_pred
        x_abs = K.abs(x)
        x_bool = K.cast(K.less_equal(x_abs, 1.0), 'float32')
        loss = 4 * K.sum(y_true[:, :, :4*num_classes] * (x_bool*(0.5*x*x) + (1-x_bool)*(x_abs-0.5))) / K.sum(epsilon + y_true[:, :, :4*num_classes])
        return loss
    return class_loss_regr_fixed_num

# 定义分类损失函数，计算分类交叉熵损失
def class_loss_cls(y_true, y_pred):
    return K.mean(categorical_crossentropy(y_true[0, :, :], y_pred[0, :, :]))

# 定义函数，根据最小边长调整图像大小
def get_new_img_size(width, height, img_min_side=600):
    if width <= height:
        f = float(img_min_side) / width
        resized_height = int(f*height)
        resized_width = int(img_min_side)
    else:
        f = float(img_min_side) / height
        resized_width = int(f*width)
        resized_height = int(img_min_side)
    return resized_width, resized_height

# 计算经过卷积网络处理后的图像输出长度
def get_img_output_length(width, height):
    # 使用卷积核大小、填充和步幅信息计算输出长度
    def get_output_length(input_length):
        filter_sizes = [7, 3, 1, 1]
        padding = [3, 1, 0, 0]
        stride = 2
        for i in range(4):
            input_length = (input_length + 2*padding[i] - filter_sizes[i]) // stride + 1
        return input_length
    return get_output_length(width), get_output_length(height)


# 定义generator类，用于生成图像数据及对应的标注数据
class Generator(object):
    # __init__方法
    def __init__(self, bbox_util, train_lines, num_classes, solid, solid_shape=[600, 600]):
        self.bbox_util = bbox_util
        self.train_line = train_lines
        self.train_batches = len(train_lines)
        self.num_classes = num_classes
        self.solid = solid
        self.solid_shape = solid_shape

    # 对图像和标注进行实时数据增强
    def get_random_data(self, annotation_line, random=True, jitter=.1, hue=.1, sat=1.1, val=1.1, proc_img=True):
        # 读取图像，并根据solid标志决定图像的目标尺寸
        line = annotation_line.split()
        image = Image.open(line[0])
        iw, ih = image.size
        if self.solid:
            w, h = self.solid_shape
        else:
            w, h = get_new_img_size(iw, ih)
        # 解析标注框的信息
        box = np.array([np.array(list(map(int, box.split(',')))) for box in line[1:]])
        # 随机调整图像的宽高比和尺寸
        new_ar = w/h * rand(1-jitter, 1+jitter) / rand(1-jitter, 1+jitter)
        scale = rand(.9, 1.1)
        if new_ar < 1:
            nh = int(scale*h)
            nw = int(nh*new_ar)
        else:
            nw = int(scale*w)
            nh = int(nw/new_ar)
        image = image.resize((nw, nh), Image.BICUBIC)
        # 将调整大小后的图像放置到背景中
        dx = int(rand(0, w-nw))
        dy = int(rand(0, h-nh))
        new_image = Image.new('RGB', (w, h), (128, 128, 128))
        new_image.paste(image, (dx, dy))
        image = new_image
        # 随机水平翻转图像
        flip = rand() < .5
        if flip: image = image.transpose(Image.FLIP_LEFT_RIGHT)
        # 【随机调整图像的色相、饱和度和亮度】
        hue = rand(-hue, hue)
        # 随机饱和度
        sat = rand(1, sat) if rand() < .5 else 1 / rand(1, sat)
        # 随机明度
        val = rand(1, val) if rand() < .5 else 1 / rand(1, val)
        # 图像从RGB转为HSV颜色空间
        x = rgb_to_hsv(np.array(image)/255.)
        # 调整色调
        x[..., 0] += hue
        x[..., 0][x[..., 0] > 1] -= 1
        x[..., 0][x[..., 0] < 0] += 1
        # 调整饱和度
        x[..., 1] *= sat
        # 调整明度
        x[..., 2] *= val
        # 限制HSV值在[0, 1]之间
        x[x > 1] = 1
        x[x < 0] = 0
        # 将图像从HSV转回RGB颜色空间中
        image_data = hsv_to_rgb(x)*255
        # 标注框调整
        box_data = np.zeros((len(box), 5))
        if len(box) > 0:
            np.random.shuffle(box)
            box[:, [0, 2]] = box[:, [0, 2]] * nw / iw + dx
            box[:, [1, 3]] = box[:, [1, 3]] * nh / ih + dy
            # 水平翻转
            if flip: box[:, [0, 2]] = w - box[:, [2, 0]]
            # 剪裁bounding boxes
            box[:, 0:2][box[:, 0:2] < 0] = 0
            box[:, 2][box[:, 2] > w] = w
            box[:, 3][box[:, 3] > h] = h
            # 筛选有效的bounding boxes
            box_w = box[:, 2] - box[:, 0]
            box_h = box[:, 3] - box[:, 1]
            box = box[np.logical_and(box_w > 1, box_h > 1)]
            # 筛选后的bounding boxes 填充到box_data
            box_data[:len(box)] = box
        if len(box) == 0:
            return image_data, []
        if (box_data[:, :4] > 0).any():
            return image_data, box_data
        else:
            return image_data, []

    # 生成数据函数
    def generate(self):
        while True:  # while true 表示会无限循环
            shuffle(self.train_lines)
            lines = self.train_lines
            for annotation_line in lines:
                img, y = self.get_random_data(annotation_line)
                height, width, _ = np.shape(img)  # 获取图片高、宽度，忽略通道数
                if len(y) == 0:
                    continue
                # 提取目标框的坐标
                boxes = np.array(y[:, :4], dtype=np.float32)
                # 将目标框标准化到[0,1]之间，便于网络处理
                boxes[:, 0] = boxes[:, 0] / width
                boxes[:, 1] = boxes[:, 1] / height
                boxes[:, 2] = boxes[:, 2] / width
                boxes[:, 3] = boxes[:, 3] / height
                # 计算每个目标框的高度和宽度
                box_heights = boxes[:, 3] - boxes[:, 1]
                box_widths = boxes[:, 2] - boxes[:, 0]
                # 跳过目标框高度或宽度小于等于0的，因为这些是无效框
                if (box_heights <= 0).any() or (box_widths <= 0).any():
                    continue
                # 标准化后的目标框坐标更新回y中
                y[:, :4] = boxes[:, :4]
                # 获取先验框，用于与目标框匹配
                anchors = get_anchors(get_img_output_length(width, height), width, height)
                # 将目标框和先验框进行匹配
                assignment = self.bbox_util.assign_boxes(y, anchors)
                # 设置每个批次中区域的数量为256
                num_regions = 256
                # 提取分类标签和回归标签
                classification = assignment[:, 4]
                regression = assignment[:, :]

                # 创建布尔掩码，标记所有正样本
                mask_pos = classification[:] > 0
                num_pos = len(classification[mask_pos])
                # 如果正样本数量超过num_regions/2,就随机选择一些正样本使得数量减少到num_region/2
                if num_pos > num_regions/2:
                    val_locs = random.sample(range(num_pos), int(num_pos - num_regions/2))
                    classification[mask_pos][val_locs] = -1
                    regression[mask_pos][val_locs, -1] = -1

                # 负样本同理
                mask_neg = classification[:] == 0
                num_neg = len(classification[mask_neg])
                if len(classification[mask_neg]) + num_pos > num_regions:
                    val_locs = random.sample(range(num_neg), int(num_neg - num_pos))
                    classification[mask_neg][val_locs] = -1

                # 调整classification和regression的形状，使其适合网络的输入
                classification = np.reshape(classification, [-1, 1])
                regression = np.reshape(regression, [-1, 5])
                # 将图片转换为Numpy数组
                tmp_inp = np.array(img)
                tmp_targets = [np.expand_dims(np.array(classification, dtype=np.float32), 0), np.expand_dims(np.array(regression, dtype=np.float32), 0)]
                yield preprocess_input(np.expand_dims(tmp_inp, 0)), tmp_targets, np.expand_dims(y, 0)
